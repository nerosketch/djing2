...
class MessageStatus(BaseAbstractModel):
    ...
    MESSAGE_STATES = (("new", _("New")), ("old", _("Seen")), ("del", _("Deleted")))
    state = models.CharField(max_length=3, choices=MESSAGE_STATES, default="new")

    def __str__(self):
        return f"{self.get_state_display()} for {self.user} ({self.msg})"
...
class ConversationMembership(BaseAbstractModel):
    ...
    PARTICIPANT_STATUS = (("adm", _("Admin")), ("gst", _("Guest")), ("ban", _("Banned user")), ("inv", _("Inviter")))
    status = models.CharField(max_length=3, choices=PARTICIPANT_STATUS, default="gst")
    who_invite_that_user = models.ForeignKey(
        UserProfile, on_delete=models.CASCADE, null=True, blank=True, related_name="self_conversations"
    )

    def __str__(self):
        return f"{self.conversation} < {self.account}"
...
class ConversationManager(models.Manager):
    def create_conversation(self, author, other_participants, title=None):
        ...
        ConversationMembership.objects.create(account=author, conversation=conversation, status="inv")
        return conversation
...
class Conversation(BaseAbstractModel):
    ...
    def new_message(self, text, attachment, author, with_status=True):
        msg = Message.objects.create(text=text, conversation=self, attachment=attachment, author=author)
        if with_status:
