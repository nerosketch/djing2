...
class TrafficArchiveManager(models.Manager):
    ...
    def get_chart_data(customer_id: int, start_date: datetime, end_date: datetime = None) -> list:
        ...
        with connection.cursor() as cur:
            cur.execute("SELECT traf_fetch_archive4graph(%s::bigint, %s, %s);", [customer_id, start_date, end_date])
...
class TrafficCache(BaseAbstractModel):
    customer = models.ForeignKey("customers.Customer", on_delete=models.CASCADE, related_name="traf_cache")
    event_time = models.DateTimeField()
