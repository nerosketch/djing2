...
class ViberMessengerModelViewSet(DjingModelViewSet):
        ...
        if isinstance(vr, ViberMessageRequest):
            ...
            if not created:
                models.ViberMessage.objects.create(
                    msg=vr.message, sender=vr.sender.id, messenger=obj, subscriber=subscriber
                )
            ...
            models.ViberSubscriber.objects.filter(uid=vr.user_id).delete()
        return Response(status=status.HTTP_200_OK)
    ...
    def make_subscriber(self, viber_user_profile: ViberUserProfile):
        subscriber, created = models.ViberSubscriber.objects.get_or_create(
            uid=viber_user_profile.id, defaults={"name": viber_user_profile.name, "avatar": viber_user_profile.avatar}
        )
    ...
    def inbox_contact(self, msg, sender: ViberUserProfile):
        ...
        if accs.exists():
            ...
            if subs_len > 0:
                ...
                viber.send_message_to_acc(
                    first_acc, gettext("Your account is attached. Now you will be receive notifications from billing")
                )
        else:
            viber.send_message_to_id(
                sender.id, gettext("Telephone not found, please specify telephone number in account in billing")
            )
