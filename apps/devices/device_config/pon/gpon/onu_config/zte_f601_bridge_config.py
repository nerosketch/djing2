...
from ..zte_utils import (
    ...
    get_free_registered_onu_number,
    zte_onu_conv_to_num,
    sn_to_mac,
    zte_onu_conv_from_onu,
    ZteOltLoginFailed,
    OnuZteRegisterError,
    ZTEFiberIsFull,
    ZteOltConsoleError,
...
def appy_config(onu_mac: str, sn: str, hostname: str, login: str, password: str, prompt: str, vlan: int):
    ...
    if choice == 0:
        ...
        unregistered_onu = get_unregistered_onu(lines=ch.get_lines_before(), serial=sn)
        ...
        ch.do_cmd(
            "show run int gpon-olt_%(stack)s/%(rack)s/%(fiber)s"
            % {"stack": stack_num, "rack": rack_num, "fiber": fiber_num},
            "%s#" % prompt,
        )
        free_onu_number = get_free_registered_onu_number(ch.get_lines_before())
        ...
        ch.do_cmd("onu %d type %s sn %s" % (free_onu_number, onu_type, sn), "%s(config-if)#" % prompt)
...
def _register_onu(
    ...
    **kwargs,
    ...
    return appy_config(onu_mac, serial, zte_ip_addr, telnet_login, telnet_passw, telnet_prompt, onu_vlan)
...
class ZteF601BridgeScriptModule(DeviceConfigType):
    ...
    def entry_point(cls, config: dict, device, *args, **kwargs) -> OptionalScriptCallResult:
        ...
        reg_dev_zte(device=device, extra_data=dict(pdev.extra_data), reg_func=_register_onu, config=config)
        return {1: "success"}
