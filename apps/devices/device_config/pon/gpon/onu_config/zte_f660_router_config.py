...
def _zte_onu_router_config_apply(
    ...
    **kwargs,
    ...
    if choice == 1:
        ...
        ch.do_cmd(f"onu {free_onu_number} profile line ZTE-F660-LINE remote ZTE-F660-ROUTER", config_if_prompt)
...
class ZteF660RouterScriptModule(DeviceConfigType):
    ...
    def entry_point(cls, config: dict, device, *args, **kwargs) -> OptionalScriptCallResult:
        ...
        zte_utils.reg_dev_zte(
            device=device, extra_data=dict(pdev.extra_data), reg_func=_zte_onu_router_config_apply, config=config
        )
        return {1: "success"}
