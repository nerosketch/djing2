...
def norm_name(name: str, replreg=None):
    if replreg is None:
        return re.sub(pattern=r"\W{1,255}", repl="", string=name, flags=re.IGNORECASE)
...
def plain_ip_device_mon_template(device) -> Optional[AnyStr]:
    ...
    parent_host_name = (
        norm_name(
            "%d%s" % (device.parent_dev.pk, translit(device.parent_dev.comment, language_code="ru", reversed=True))
        )
