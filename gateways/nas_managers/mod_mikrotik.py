...
from djing2.lib.decorators import LazyInitMetaclass
from gateways.nas_managers import core
from gateways.nas_managers import structs as i_structs
...
class ApiRos(object):
    ...
    def write_bytes(self, s):
        ...
        while n < len(s):
            ...
            if r == 0:
                raise core.GatewayFailedResult("connection closed by remote end")
            n += r
...
class MikrotikTransmitter(core.BaseTransmitter, ApiRos,
    ...
    def __init__(self, login: str, password: str, ip: str, port: int,
        ...
        if not enabled:
            raise core.GatewayFailedResult(_('Gateway disabled'))
        try:
            ...
            raise core.GatewayNetworkError('Connection to %s is Refused' % ip)
    ...
    def _exec_cmd(self, cmd: Iterable) -> Dict:
        ...
        for k, v in self.talk_iter(cmd):
            if k == '!done':
                ...
                raise core.GatewayFailedResult(v.get('=message'))
            r[k] = v or None
        return r
    ...
    def add_user(self, queue: i_structs.SubnetQueue, *args):
        ...
        except core.GatewayFailedResult as e:
            print('Error:', e)
        net = queue.network
        if not issubclass(net.__class__, _BaseNetwork):
            raise TypeError
